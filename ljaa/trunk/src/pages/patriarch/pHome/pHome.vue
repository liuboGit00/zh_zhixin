<template>
    <div class="patriarch">
        ...
        <div v-if="step!=1">
            <div class="hello_img">
                ...
                <div v-transfer-dom>
                    ...
                    <!--<confirm v-model="show1" @on-confirm="change" :showCancelButton="false" v-if=" === '' && commonList !== []" >-->
                        <!--<ul>-->
                            <!--<li>-->
                        ...
                        <!--</ul>-->
                    <!--</confirm>-->
                      <!--v-else-if="parentStatus === 'change' && commonList !== []"-->
                      <!--style="max-height: 1rem;overflow: auto;"-->
                      <confirm v-model="show1" @on-cancel="onCancel" @on-confirm="change">
                        <ul>
            ...
            <div class="homework ">
                ...
                <div v-for="(item,index) in dataWorkList" :key='item.index'>
                    ...
                    <div class="list-content" v-for='(item1,index1) in item' :key='item1.index1'>
                        <button v-if="item1.isFinished" @click="openWork('homework',item1)" class="green" mini>已完成
                        </button>
            ...
            <div class="homework ">
                ...
                <div v-for="(item,index) in dataTestList" :key='item.index'>
                    ...
                    <div class="list-content" v-for='(item1,index1) in item' :key='item1.index1'>
                        <button @click="openWork('test',item1)" v-if="item1.answerStatus" class="green" mini>已完成</button>
        ...
        <alert v-model="alertShow" :hideOnBlur="false">{{message}}</alert>
      <!--@on-hide="alertAdd"-->
      <!--<alert v-model="alertShowData" :hideOnBlur="false" @on-hide="alertAdd">确认加载绑定学生的最新数据吗？</alert>-->
    </div>
</template>
<script>
  import api from './api'
  ...
  export default {
    data () {
      return {
        ...
        commonList: [],
        show1: false,
        // alertShowData:false,  //首次更新数据
        data: [],
        productList: [],
        login: {}, // 获取登录
        changeStatus : '', //切换学生时的状态
        parentStatus:'', //家长登录成功 请求学生绑定信息
      }
    },
    methods: {
      openTotAppraisal () {
      ...
      openWork (type, item) {
        this.$router.push({
          name: 'homeworkRes',
          params: {id: item.id, type: type, studentId: this.info.id},
...
//        this.$router.push({name: 'login'})
      ...
      onShow () {
        this.studentIds = ''
        // this.radioValue[0] = this.info.account
        ...
        //   }
        // })
        // console.log(this.radioValue,'onShow')
      },
      onCancel () {
        // this.radioValue[0] = this.info.account
        // console.log(this.radioValue[0],'on onCancel',this.info)
      },
      change (val, label) {//切换学生
        ...
        this.commonList.map((item) => {
          if (item.account == this.radioValue[0]) {
            this.$store.commit('setLoginInfo', item)
            this.info = item
            this.$store.state.loginInfo = this.info
      ...
      //点击绑定
      add () {
        console.log(this.studentIds, 321321)
        if (!this.studentIds) {
          this.alertShow = true
          return this.message = '学生编号不能为空，请重新操作'
        } else {
          this.ajax({
      ...
      queryGuardianStudent () {//查询家长学生关系
        this.ajax({
          ajaxSuccess: (data, res) => {
            ...
            // }  else {
            //   this.info = this.$store.state.loginInfo;
            //   console.log(this.info,'this.info   666')
            // }
          ...
          params: {
            currentStudentId: this.changeStatus === 'change' ? this.info.id : ''
          }
        })
      }
    },
