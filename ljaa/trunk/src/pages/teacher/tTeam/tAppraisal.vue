    ...
    <div class="tAppraisal">
        ...
        <ul class="tAppraisalCircle">
            <li v-for="(item,index) in data.answerInfoList" :key="index">
                <div v-if="!item.isCorrecting" style="background: rgb(0,155,219)"
                ...
                <!--{{index+1}}-->
                <!--</div>-->
                <div v-if="item.isCorrecting&&item.answerResult==true" style="background: rgba(26,173,25,1)"
                     @click="open('tAppraisalContent',index+1)">
                    {{index + 1}}
                </div>
                <div v-if="item.isCorrecting&&item.answerResult==false" style="background: #FB4440;"
                     @click="open('tAppraisalContent',index+1)">
                    {{index + 1}}
                </div>
            </li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail () {
        this.ajax({
          ajaxSuccess: (data, res) => {this.data = data; console.log(data)},
