...
<template>
  <div class="tNone">
    <div class="harryUp" v-for="(item,index) in data" :key="index">
        <div class="item">
          <span>{{item.studentName}}</span>  <x-button mini type="primary" :disabled="time > item.submitTime"  @click.native="sendUrgeWork(item)">催交</x-button>
        </div>
    </div>
    <div style="margin: 0.1rem 0.15rem">
    <x-button v-if="data.length" @click.native="to_work" :disabled="time > data[0].submitTime"  type="primary">全部催交</x-button>
    </div>
