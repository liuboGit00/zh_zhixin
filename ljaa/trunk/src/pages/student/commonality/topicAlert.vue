...
<template>
    <div class="topicAlert-root">
        <div class="tab_click">
            <div @click="change(1)" v-if="!$route.query.seeType">题版
                <div v-transfer-dom class="persistent">
                    <confirm v-if="typeTestHome === 'test'" v-model="show1" title="题版" @on-cancel="onCancel" :close-on-confirm="temp" @on-confirm="onConfirm" @on-show="onShow"
                             @on-hide="onHide">
                        ...
                        <div   v-for="(item,index) in data" :key="index">
                            ...
                            <div class="recentPractice_last_grid">
                              <div v-for="(items,index) in item.child" :key="index">
                                <div class="grid-center orange"
                                     :class="skip==items.seq? 'active':''" @click="open(items)">
                                  {{items.seq}}
                                    </div>
                                </div>
                    ...
                    <confirm v-else v-model="show1" title="题版" @on-cancel="onCancel" :close-on-confirm="temp" @on-confirm="onConfirm" @on-show="onShow"
                           @on-hide="onHide">
                    <div style="max-height:3rem;overflow: auto" >
                      <div   v-for="(item,index) in data" :key="index">
                        <div class="recentPractice_last_title">{{item.name}}</div>
                        <div class="recentPractice_last_grid">
                          <div v-for="(items,index) in item.child" :key="index">
                            <!--{{items.isAnswer}}{{items.answerResult}}-->
                            <!--<div class="grid-center" v-if="items.isAnswer"-->
                            <!--:class="skip==items.seq? 'active':''" @click="open(items)">-->
                            <!--{{items.seq}}-->
                            <!--</div>-->
                            <div class="grid-center orange" v-if="!items.isAnswer"
                                 :class="skip==items.seq? 'active':''" @click="open(items)">
                              {{items.seq}}
                            </div>
                            <div class="grid-center green" :class="skip==items.seq? 'active':''"
                                 v-else-if="(items.isAnswer&&items.answerResult)==true" @click="open(items)">
                              {{items.seq}}
                            </div>
                            <div class="grid-center defaultColor" :class="skip==items.seq? 'active':''"
                                 v-else-if="items.isAnswer&&items.answerResult===null" @click="open(items)">
                              {{items.seq}}
                            </div>
                            <div class="grid-center red" :class="skip==items.seq? 'active':''" v-else
                                 @click="open(items)">
                              {{items.seq}}
                            </div>
                  ...
                  <div   v-for="(item,index) in data" :key="index">
                    ...
                    <div class="recentPractice_last_grid">
                      <div v-for="(items,index) in item.child" :key="index">
                        ...
                        <!--{{items.seq}}-->
                        <!--</div>-->
                        <!--{{items.isAnswer}}isAnswer{{items.answerResult}}answerResult-->
                        <div class="grid-center orange" v-if="!items.isAnswer"
                             :class="skip==items.seq? 'active':''" @click="open(items)">
                          {{items.seq}}
                        </div>
...
import { debug } from 'util';
  ...
  let Util = null
  export default {
    props:['questionsId','questionList','typeTestHome'],
    ...
    methods: {
      ...
      open (type) {
        ...
        if (!type.isAnswer && ['1', '4', '5'].includes(this.$route.query.type)) {
             this.temp=false;
          }else{
            this.temp=true;
          }
...
//        }
      ...
      onShow () {
        ...
        this.$emit('questionNextList','questionNextList');
        console.log('on show',this.$refs.questionList)
      },
      onItemClick () {
