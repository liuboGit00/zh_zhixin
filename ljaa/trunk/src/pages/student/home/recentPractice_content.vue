    ...
    <div class="recentPractice_content">
        ...
        <ul class="recentContent" v-for="(item,index) in paperList" :key="index">
            <li >
                ...
                <div class="progress1">
                    <div>进度</div>
                    <x-progress id="progress1_x" :percent="Number((item.answerNums/item.questionNums*100).toFixed(1))"
                                :show-cancel="false"></x-progress>
                    <div>{{(item.answerNums/item.questionNums*100).toFixed(1)+ '%'}}</div>
                </div>
            </li>
      ...
      getStudyProductDetail()  {
        this.ajax({
          ajaxSuccess: (data, res) => {
            this.data = data
            this.data.icon = this.getStaticPath(this.data.icon)
          },
