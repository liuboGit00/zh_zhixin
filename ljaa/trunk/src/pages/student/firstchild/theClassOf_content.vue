        ...
        <scroller :height="top"  lock-x  @on-scroll-bottom="onScrollBottom" ref="scrollerBottom"
            ...
            <div  >
                ...
                <div class="thisWeek" v-for="(item,index) in  workData" :key="index">
                    <div>
                        布置时间：{{(item.startTime || item.releaseTime) | formatDate('yyyy-MM-dd HH:mm')}}
                    </div>
                    <ul>
                        <li>
                            <div><span v-if="selectTab">测评</span>
                              <span v-else>作业</span>名称:{{item.title||item.name}}</div>
                            <div>
                                上交时间：{{(item.submitTime || item.endTime) | formatDate('yyyy-MM-dd HH:mm')
                                }}
                            </div>
                            <div>共{{item.questionsNums || item.totalQuestions}}小题 {{item.submitNum || item.finishedNums||0}}人提交
                            </div>
                        </li>
