        ...
        <div class="border" v-if="selectTab=='self'" style="margin-top: -0.01rem">
            ...
            <div style="width: 1.4rem;margin-top: 0.28rem;margin-left: 0.95rem">
                ...
                <p style="text-align: center;font-size: 0.12rem;color: #c3c4c4;">
                  用时：{{parseInt(UserEvaluationInfo.usedTime / 60 || 0)}}分钟{{(UserEvaluationInfo.usedTime % 60 || 0)}}秒 |
                  成绩：{{UserEvaluationInfo.score}}分
            ...
            <div style="margin:0 0.12rem;margin-right: 0">
                ...
                <p class="detial">
                    <span v-for="(item,index) in UserEvaluationInfo.answerInfoList">
                      <!--:class="[item.studentAnswer? 'red ':'defaultColor']"-->
                         <span @click="startAnswer(item)" v-if="item.isCorrecting === null " class="orange">
                            <span style="color: #fff">{{item.questionsSeq}}</span>
                         </span>
                         <span @click="startAnswer(item)" v-else-if="item.isCorrecting === 0" class="defaultColor">
                              <span style="color: #fff">{{item.questionsSeq}}</span>
                         </span>
                          <span class="defaultColor" v-else-if="item.isCorrecting === 1"
  ...
  methods: {
    ...
    init () {
      ...
      if (this.$route.params.type != 'homework') {
        ...
        this.params = {papersId: this.$route.params.id, studentId: this.$route.params.studentId || this.$store.state.loginInfo.id}
        ...
        this.params = {workId: this.$route.params.id, groupsId: this.$route.query.groupsId, studentId: this.$route.params.studentId || this.$store.state.loginInfo.id}
        this.assessmentId = 1
        this.types = 5
      }
      // setTimeout(() => {
