...
let mutations = {
  ...
  initData: (state, infoData) => {
    ...
    if (infoData.room && infoData.room.specialty && typeof(infoData.room.specialty) == 'string') {
      console.log(typeof(infoData.room.specialty),999)
      state.room.specialty=infoData.room.specialty.split(',')
    }
    // if(infoData.room&&infoData.room.spcialty){
    //   state.room.spcialty = spcialty
    // }
    console.log(state.room,311331)
