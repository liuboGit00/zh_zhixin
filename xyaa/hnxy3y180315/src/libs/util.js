      ...
      data () {
        ...
        return {
          ...
          pickerOptions2: {
            //选择开始时间后设置结束日期的限制
            disabledDate: (time) => {
              let starTimesEnd = new Date((new Date()).getFullYear() + '-01'); // 开始时间最小
              let endTimesEnd = new Date((new Date()).getFullYear() + '-12'); // 结束时间最大

              let flag = false;
              if(this.endTimes){
                if(time.getTime() > this.endTimes || time.getTime() < starTimesEnd){
                  flag = true;
                }
              }else{
                if(time.getTime() > endTimesEnd || time.getTime() < starTimesEnd){
                  flag = true;
                }
              }
          ...
          pickerOptions3: {
            //选择结束时间后设置开始日期的限制
            disabledDate: (time) => {
              let starTimesEnd = new Date((new Date()).getFullYear() + '-01-01'); // 开始时间最小
              let endTimesEnd = new Date((new Date()).getFullYear() + '-12-30'); // 结束时间最大

              let flag = false;
              if(this.starTimes){
                if(time.getTime() < this.starTimes || time.getTime() > endTimesEnd){
                  flag = true;
                }
              }else{
                if(time.getTime() > endTimesEnd || time.getTime() < starTimesEnd){
