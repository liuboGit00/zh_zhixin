      ...
      <el-row>
        ...
        <el-col :span="17" :offset="2" v-if="isPictureNews">
          <el-form-item label="标题图片:" prop="titlePicturePath" class="feildFontweight is-required">
            <upload-photo-wall v-if="fileList && fileList.length" type="picture" :actionUrl="'/file/upload/static'" :fileList="fileList"
      ...
      <el-row v-if="show">
        <el-col :span="17" :offset="2">
          <el-form-item type="附件" label="内容:" class="feildFontweight">
            <viewUEditor style="width:600px;" :name="'ud1'" @storeUE="storeUE" @getUeditorVal="getUeditorVal" :ueditor-val="ueditorVal"
            ...
            <el-row v-if="data.multimediaFileList!=0">
              ...
              <el-col :span="7">
                <ul style="height:300px;background: #cecece;overflow: auto">
                  <li v-for="(item,i) in data.multimediaFileList" :key="i" :class="{'my-video-list':(index==i)}" @click="changeIndex(item,i)"
                      style="color: #fff;height:40px;line-height: 40px;cursor: pointer;" class="overflow-txt1">{{item.fileName}}
                  </li>
                </ul>
              </el-col>
            </el-row>
...
<script>
  ...
  export default {
    ...
    components: {
      viewUEditor, myVideo, uploadPhotoWall
    }
  }
</script>
