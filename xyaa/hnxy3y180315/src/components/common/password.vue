    ...
    data() {
      ...
      var validatePass = (rule, value, callback) => {
        if (value === '') {
          ...
          // }
          // callback();
          // let newRules = /^(?![0-9]+$)(?![a-zA-Z]+$)(?!([^(0-9a-zA-Z)]|[])+$)([^(0-9a-zA-Z)]|[]|[a-zA-Z]|[0-9]|[()]){8,}$/
          let newRules = /^(?=.*?[a-z])(?=.*?[A-Z])(?=.*?\d)(?=.*?[!#@*&.])[a-zA-Z\d!#@*&.]{6,}$/;
          if(newRules.test(value) === false){
            callback(new Error('密码必须由大写字母、小写字母、特殊字符及数字四种组成，且密码长度不能少于6个字符'));
          }else{
            if (this.ruleForm2.checkPass !== '') {
              this.$refs.ruleForm2.validateField('checkPass');
            }
