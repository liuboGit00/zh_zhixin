...
<template>
  <div>
    <div class="cal-schoolTit">{{operailityData["depName"]}}：{{operailityData["ts"]}}{{ operailityData.podClass == 'SXS' ? '周' : '月' }}</div>
    <br />
    <el-table
      v-for="valItem in 1"
      :key="valItem"
          ...
          <div v-if="scope.$index==0">
            <el-form :model="{ts:formValidate.depTs}" ref="formValidate" :rules="rules" label-width="0" >
              <el-form-item  prop="ts">
                <el-input style="width: 60px;" @change="handleRandomInput('depTs')" :disabled="formValidate.cType==0" v-model="formValidate.depTs" placeholder="请输入内容"></el-input>{{ operailityData.podClass == 'SXS' ? '周' : '月' }}
              </el-form-item>
            </el-form>
          </div>
          <div v-else>
            <el-form :model="{ts:scope.row.depTs}" ref="formValidate" :rules="rules" label-width="0" >
              <el-form-item  prop="ts">
                <el-input style="width: 60px;" @change="handleMustInput('depTs')" :disabled="formValidate.cType==1" v-model="scope.row.depTs" placeholder="请输入内容"></el-input>{{ operailityData.podClass == 'SXS' ? '周' : '月' }}
              </el-form-item>
            </el-form>
          </div>
        </template>
