...
<template>
  <div id="content" ref="content" style="position: absolute;top: 10px;left: 10px;right:10px;bottom:10px;overflow: auto">
    ...
    <show v-else :operailityData="formValidate"></show>
  </div>
</template>
<script>
  /*当前组件必要引入*/
  ...
  * */
      listenSubEvent(isLoadingFun,messTitle,status=''){
        ...
        if(isSubmit && (this.ergodic(this.formValidate.directionList))&& (this.ergodic(this.formValidate.curriculumLists[0]))&& (this.ergodic(this.formValidate.curriculumLists[1]))&& (this.ergodic(this.formValidate.curriculumLists[2]))&& (this.ergodic(this.formValidate.curriculumLists[3]))&& (this.ergodic(this.formValidate.bookList))){
