...
<template>
  <div id="content" ref="content">
    <div style="margin-bottom: 20px;">
      <span style="font-size: 21px; overflow: hidden">已选择导师：</span>
      <el-tag
        v-if="selectUserMentor!=0"
        v-for="(tag,index) in selectUserMentor"
        :key="index"
        :closable="!tag.closable"
        :type="tag.teacherName"
        style="margin: 5px"
      ...
      <!--<span style="font-size: 16px;margin-left: 30px;color: red">注：只能选择两个导师</span>-->
      <br/>
      <el-button style="margin-top: 10px" type="primary" @click="confirm" :disabled="style.confirm">确定</el-button>
    </div>
    <!--- 开放导师列表 --->
    <div>
      <!--表格数据-->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <!--<el-button v-else size="small" @click="" :disabled="true">选择</el-button>-->
      ...
      <Modal
        ...
        <user-info v-if="showModal" @cancel="cancel" @show="subCallback" :operaility-data="{id:operailityData.teacherId}"></user-info>
        <div slot="footer"></div>
      </Modal>
    </div>
    ...
    export default {
        ...
        methods: {
          ...
          confirmShow(){
            ...
            if(!this.isEdit){
              if(selectUserMentor.length===this.getSelectUserMentor.length){
                this.style.confirm = true;
              } else if(selectUserMentor.length>2){
                this.style.confirm = true;
              }else {
                this.style.confirm = false;
              }
            }else {
              if(selectUserMentor.length>2){
                this.style.confirm = true;
              }else {
                this.style.confirm = false;
              }
          ...
          selectable(row){
            ...
            if(getSelectUserMentor.length>=2){
              return false;
            }else if (getSelectUserMentor.length==1){
              let id=getSelectUserMentor[0].teacherId;
              if(id+'' == row.teacherId+''){
                return false;
              }else {
                return true;
