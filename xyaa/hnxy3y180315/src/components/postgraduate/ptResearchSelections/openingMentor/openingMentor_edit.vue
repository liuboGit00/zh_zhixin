<template>
  <div>
        ...
        <el-row>
          ...
          <el-col :offset="2">
            <el-form-item label="选择导师:" >
              ...
              <div style="white-space:normal;" v-if="mentorArr.length>0" v-model="formInline.teacherIds" class="tip">{{mentorArr.join(',')}}</div>
            </el-form-item>
          </el-col>
          ...
          <el-col :offset="2">
            <el-form-item label="选择学员:" >
              ...
              <div style="white-space:normal;" v-if="studentArr.length>0" v-model="formInline.studentIds" class="tip" >{{ studentArr.join(',')}}</div>
            </el-form-item>
          </el-col>
          <el-col :offset="2">
            <el-form-item label="开放时间:" prop="admissionDate">
              <el-date-picker
    ...
    <Modal
      ...
      <selectU v-if="hostModal" @cancel="hostModal=false" :isOnlyOne="false" :url="treeUrl" :ajaxData="{rootId:-102}" @setUsers="selectHostCallback" :initUser="selectUserMentor"></selectU>
      <div slot="footer"></div>
    </Modal>
    <!--选择学生-->
    <Modal
    ...
    >
      <modal-header slot="header" :content="hostId"></modal-header>
      <select-user v-if="studentModal" @cancel="studentModal=false" :url="url" :isOnlyOne="false" @setUsers="selectHostCallback1" :initUser="selectUserStudent"></select-user>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
...
//            if (isSubmit) {
            ...
            formInline.majorCode = major[1];
              this.editMessTitle.ajaxParams.data = this.formDate(this.getFormData(formInline),['startTime','endTime'],'yyyy-MM-dd');
