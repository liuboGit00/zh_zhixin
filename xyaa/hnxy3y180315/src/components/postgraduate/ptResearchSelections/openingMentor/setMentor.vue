<template>
  ...
  <div>
          ...
          <el-row>
            ...
            <el-col :offset="2">
              <el-form-item label="导师" >
                ...
                <div v-if="mentorArr.length>0" v-model="formInline.teacherIds" class="tip">{{mentorArr.join(',')}}</div>
              </el-form-item>
            </el-col>
            ...
            <el-col :offset="2">
              <el-form-item label="学员">
                ...
                <div v-if="studentArr.length>0" v-model="formInline.studentIds" class="tip" >{{ studentArr.join(',')}}</div>
              </el-form-item>
            </el-col>
            <el-col :offset="2">
    ...
    >
      ...
      <selectU @cancel="hostModal=false" :url="treeUrl" :isOnlyOne="false" :ajaxData="{rootId:-102}" @setUsers="selectHostCallback" :initUser="selectUserMentor"></selectU>
      <div slot="footer"></div>
    </Modal>
    <!--选择学生-->
    <Modal
    ...
    >
      <modal-header slot="header" :content="hostId"></modal-header>
      <select-user @cancel="hostModal1=false" :url="url" :isOnlyOne="false" @setUsers="selectHostCallback1"  :initUser="selectUserStudent"></select-user>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    methods:{
      ...
      selectHostCallback(data){
        ...
        let ids=this.formInline.teacherIds.split(',');
        let names=[];
        data.map(item =>{
      ...
      selectHostCallback1(data){
        let ids= this.formInline.studentIds.split(',') ;
        let names=[];
        data.map(item =>{
       ...
       * */
      majorDefault(item){
        this.formInline.major = item.name+'-'+item.code;
        this.formInline.majorCode = item.code;
