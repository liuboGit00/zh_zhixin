...
<template>
  <layout-tree>
    ...
    <template v-if="['ZYY','SXS','YJS','JXS','BKS'].indexOf(viewTypes) > -1">
      <!-- 住院医人员管理 -->
      <zyy-content v-if="viewTypes === 'ZYY'" slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></zyy-content>
      <!-- 实习生人员管理 -->
      <sxs-content v-if="viewTypes === 'SXS'" slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></sxs-content>
      <!-- 研究生人员管理 -->
      <yjs-content v-if="viewTypes === 'YJS'" slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></yjs-content>
      <!-- 进修生人员管理 -->
      <jxs-content v-if="viewTypes === 'JXS'" slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></jxs-content>
      <!-- 本科生人员管理 -->
      <bks-content v-if="viewTypes === 'BKS'" slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></bks-content>
    </template>
    <!-- 默认部门人员管理 -->
    <dep-content v-else slot="right" admin="true" :deptName="deptName" :deptId="deptId" @refreshTree="refreshTree"></dep-content>
  </layout-tree>
</template>
<script>
  /*当前组件必要引入*/
