...
export default {
  ...
  mounted() {
    ...
    this.showTime = this.checktime(Math.floor(url.timeLong / 3600)) +" : " +  this.checktime(Math.floor(url.timeLong / 60)) +" : " + this.checktime(Math.floor(url.timeLong % 60));
    this.runTime = url.timeLong
    //判断是否正在考试中
    this.judgeTimer = setInterval(this.judgeExam,1000)
  },
  methods: {
    judgeExam(){
      let params = {
    ...
    judgeExamAjaxSuccess(res){
      let status = res.data.stationStatus
      if(status == 'YAP'){
    ...
    startExam() {
      if (!this.isExam) {   //点击开始考试
        // this.$alert('确定开始考试吗？', '确认', {
        //   confirmButtonText: '确定',
        //   customClass: 'roomControl',
        //   callback: action => {
