...
<script>
  ...
  export default {
    //props接收父组件传递过来的数据
    props: ['operailityData', 'deptId','treeRoad'],
    data () {
      return {
        active: 0,
        //保存按钮基本信息
        excelImportUrl: api.excelImport.path + this.treeRoad,
        //导入条件
        loadBtn: {
          title: '提交',
        ...
        format: {
          ...
          '性别': 'sex',
          '身份证号': 'idCard',
          '准考证号': 'quasiExamNumber',
          '考试日期': 'examinationDate',
          '考试时间': 'examinationTime',
          '考试类别': 'examinationType',
        },
    ...
    mounted () {
      ...
      this.http = http + 'static/template/考生信息模板.xls';
      this.init();
    },
    methods: {
      /*
