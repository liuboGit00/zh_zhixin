  ...
  <div>
    <el-row class="editForm">
      ...
      <el-table :maxHeight="300" :data="formValidate" tooltip-effect="dark" align="center">
        ...
        <el-table-column label="交通费" prop="trafficFee">
...
//          console.log(this.getFormData(this.formValidate))
      ...
      // 检查数据
      checkData() {
        let keys = {trafficFee: "交通费", conferenceFee: '会务费', travelFee: '差率费', hotelFee: '住宿费'};
        for (let i = 0, l = this.formValidate.length; i < l; i++) {
          for (let key in keys) {
            if (!/^(0|[1-9](\d+|))$/.test(this.formValidate[i][key])) {
              this.errorMess(`${this.formValidate[i].userName} 的 ${keys[key]} 必须是有效的整数`)
              return false
            }
          }
        }
