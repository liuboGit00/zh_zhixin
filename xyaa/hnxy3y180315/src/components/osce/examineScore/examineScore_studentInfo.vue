<template>
  ...
  <div class="exmUserContent" style="min-height:600px;">
    ...
    <el-row v-if="studentInfo.randomType === 'TEACHER_SELECT' && !studentInfo.contentId">
      <el-form ref="selectContentForm" :model="selectContentForm" label-width="80px" :inline="true" :rules="selectContent">
        <el-form-item label="评分表：" prop="contentId">
          <el-select v-model="selectContentForm.contentId" placeholder="请选择评分表" @change="getPriveContent" :disabled="selectContentForm.select">
            <el-option v-for="item in selectContentOption" :key="item.id" :label="item.scoreTableList&&item.scoreTableList[0]&&item.scoreTableList[0].scoreTableName" :value="item.contentId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <div class="exmContentMain">
      <el-form ref="formValidate" label-width="80px">
        ...
        <el-table border align="center" :height="tableMaxHeight" show-summary
                  ...
                  v-if="contentDataList.detailsList.length>0"
        ...
        <!--为了兼容抽题类型为“考官随机抽题”时，不显示总分一列-->
        <div v-else>
          <form class="el-form">
            <div
              class="el-table add-scope el-table--fit el-table--border el-table--enable-row-hover el-table--enable-row-transition"
              ...
              <div class="el-table__header-wrapper">
                <table cellspacing="0" cellpadding="0" border="0" class="el-table__header" style="width: 899px;">
                  <colgroup>
                    <col name="el-table_1_column_271" width="150">
                    <col name="el-table_1_column_272" width="552">
                    <col name="el-table_1_column_273" width="80">
                    <col name="el-table_1_column_274" width="100">
                    <col name="gutter" width="">
                  </colgroup>
                  <thead>
                  <tr>
                    <th colspan="1" rowspan="1" class="el-table_1_column_271 is-leaf">
                      <div class="cell">项目</div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_272 is-left is-leaf">
                      <div class="cell">评分标准及要求</div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_273 is-right is-leaf">
                      <div class="cell">标准分</div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_274 is-right is-leaf">
              ...
              <div class="el-table__body-wrapper" style="height: 318px;"></div>
              <div class="el-table__footer-wrapper">
                <table cellspacing="0" cellpadding="0" border="0" class="el-table__footer" style="width: 899px;">
                  <colgroup>
                    <col name="el-table_1_column_271" width="150">
                    <col name="el-table_1_column_272" width="552">
                    <col name="el-table_1_column_273" width="80">
                  ...
                  <tbody>
                  <tr>
                    <td colspan="1" rowspan="1" class="el-table_1_column_271 is-leaf">
                      <div class="cell">总分</div>
                    </td>
                    <td colspan="1" rowspan="1" class="el-table_1_column_272 is-left is-leaf">
                      <div class="cell"></div>
                    </td>
                    <td colspan="1" rowspan="1" class="el-table_1_column_273 is-right is-leaf">
                      <div class="cell">100</div>
                    </td>
                    <td colspan="1" rowspan="1" class="el-table_1_column_274 is-right is-leaf">
                      <div class="cell">0</div>
                    </td>
                    <td class="gutter" style="width: 0px;"></td>
                  </tr>
                  </tbody>
                </table>
              </div><!----><!----><!---->
    ...
    <div class="studentInfo">
      ...
      <div class="studentInfoBox">
        ...
        <p class="otherInfo"  v-if="(new Date().getTime() < new Date(studentInfo.startTime).getTime()) && sceneType==='STANDARD'">开考倒计时：{{ startCountDown }}</p>
        <p class="otherInfo" v-else>结束倒计时：{{ countDown }}</p>
      ...
      <p align="center" style="margin-top:20px;">
        ...
        <el-button v-if="studentInfo.examStatus === 'ONGOING'" :disabled="studentInfo.randomType === 'TEACHER_SELECT' && !studentInfo.contentId && !selectContentForm.select"
                   type="danger" @click="finishAndSub">考核结束提交分数
        </el-button>
        <el-button v-if="!chgScore && studentInfo.examStatus === 'FINISH'"
                   :disabled="studentInfo.randomType === 'TEACHER_SELECT' && !studentInfo.contentId && !selectContentForm.select" type="danger" @click="changeScore">修改分数
        </el-button>

        <load-btn @listenSubEvent="listenSubEvent" type="success" v-if="chgScore" :btnData="loadBtn"></load-btn>
        <el-button v-if="!studentInfo.contentId && studentInfo.randomType !== 'TEACHER_SELECT'" type="warning" @click="sample">抽题</el-button>
      </p>
...
<script>
  ...
  export default {
    ...
    props: ['sceneId', 'stationId', 'roomId', 'userSum', 'teacherId','sceneType'],
    data () {
      return {
        rules,
        selectContent,
    ...
    methods: {
      ...
      getCountDown () {
        ...
        sd = setInterval(() => {
          // console.log(this,'************')
          if(this.studentInfo.sceneStatus === 'PUBLISH')return//未开始不计时
          let nowTime = new Date().getTime();
          ...
          this.ajax({
            ajaxSuccess: (res) => {
              ...
              if (res.data == 'ONGOING') {
                _this.countDown = _this.getTime(endTime - nowTime)
                 _this.startCountDown = _this.getTime(new Date(this.studentInfo.startTime).getTime() - nowTime)
                 console.log(_this.startCountDown)
              }
              // clearInterval(sd);
              // data = data.child
              // data.unshift({
            ...
            ajaxParams: {
              url: '/scene/quertRedisScene/'+this.sceneId,  //向后台请求数据的地址
              // params:api.getByCode.path,
              method:'get',
            },
          })
         
          // console.log(_this.startCountDown , nowTime ,this.studentInfo.startTime)
