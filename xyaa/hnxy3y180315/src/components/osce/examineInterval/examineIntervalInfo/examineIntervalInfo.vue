<template>
  ...
  <div class="roomHoverInfo">
    <div class="basicInfo">
      ...
      <span v-else :class="{'noData':!basicsTime}">考试时间：{{sceneType == 'STANDARD' ? (info.timeCount * basicsTime).toFixed(1) : (info.timeCount).toFixed(1)}}分钟</span>
      ...
      <template v-if="info.stationType === 'SP' || info.randomType == 'NO'">
        ...
        <el-table-column label="考核时间" prop="timeCount" show-overflow-tooltip>
          <template scope="scope">
            ...
            {{sceneType == 'STANDARD' ? scope.row.timeCount * basicsTime : scope.row.timeCount}}
          </template>
        </el-table-column>
      </template>
    </el-table>
