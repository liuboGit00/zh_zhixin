  ...
  <div class="editForm">
    <el-form :model="info" ref="info" label-width="120px" class="resetFormItemHeight esBasicInfo">
      <el-row>
        ...
        <el-col :span="8" :offset="2">
          <el-form-item label="考站类型：" prop="stationType">
            <el-select v-model="info.stationType" filterable placeholder="请选择" @change="changeStationType">
              <template v-for="item in typeOption" v-if="item.value !== ''&&info.isTeacherContent!=='NO'">
              ...
              <template v-for="item in typeList" v-if="item.value !== ''&&info.isTeacherContent=='NO'">
                <template v-if="sceneType === 'FREE'">
                  <el-option v-if="item.value !== 'REST'" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </template>
        ...
        <template v-if="info.stationType==='ORDINARY'&&info.isTeacherContent!=='NO'">
          <el-col :span="8" :offset="2">
        ...
        <el-col :span="8" :offset="2" v-if="!['REST'].includes(info.stationType)">
          <el-form-item label="专业：" prop="randomType">
            <el-select v-model="room.specialty" multiple placeholder="请选择">
        ...
        <el-col align="center" style="margin-top:10px;">
          <el-button type="success" @click="next">{{ info.stationType=='REST'||info.isTeacherContent=='NO' ? '完成' : '下一步' }}</el-button>
        </el-col>
      </el-row>
    </el-form>
...
//          },
    ...
    methods: {
      ...
      next () {
        ...
        if (this.info.stationType == 'REST'||this.info.isTeacherContent=='NO') {
          this.$emit('done');
        } else {
          this.$emit('goStep', 2);
        }
    ...
    created () {
      ...
      console.log('temp', temp, info,this.$store.state)
      this.info = temp.info; // 考站信息
      this.info.isTeacherContent=this.$store.state.examineInterval.station.info.isTeacherContent
      temp.room.specialty = temp.room.specialty || []//把字符串变为数组
      ...
      this.specialtyList =this.specialtyList.join(',').split(',')
      console.log(this.specialtyList, this.room)
