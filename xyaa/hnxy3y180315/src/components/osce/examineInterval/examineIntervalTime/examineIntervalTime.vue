  ...
  <div class="examineIntervalTime">
    ...
    <div class="exContent">
      <div class="exLeftMenu">
        ...
        <p v-if="this.$store.state.examineInterval.station.info.isTeacherContent!='NO'" :class="{'active':active === 'teacher'}" @click="show('teacher')">监考老师时间表</p>
        <!--<p :class="{'active':active === 'sp'}" @click="show('sp')" v-if="viewType === 'STANDARD'">SP时间表</p>-->
        <p v-if="this.$store.state.examineInterval.station.info.isTeacherContent!='NO'" :class="{'active':active === 'sp'}" @click="show('sp')">SP时间表</p>
      </div>
      ...
      setCheckData(){
        let opt = {
          ajaxSuccess: res => {
            /* var  tempItem=true;
             let temp;
             if(this.$store.state.examineInterval.station.info.isTeacherContent!='NO'){
               temp={
                 contNum:'考核内容',
 //              stationName: "22"
 //                teacherNum:'监考老师',
             ...
             let messArr=[];
              res.data.map((item)=>{
                ...
                if(arr.length){
                  messArr.push(item.stationName+'考站的'+arr.join(',')+'不能为空')
                }
              })
             if(!tempItem){
               messArr.push('参考人员不能为空')
             }
              if(messArr.length){
                this.errorMess(messArr.join(';'))
                return
              }*/
            let opt = {
              successTitle: '生成成功',
      ...
      // 设置时间表的数据
      setTimeData (res) {
        // this.$store.state.examineInterval.station.info.setupTime=res.data.setupTime
        this.setupTime = res.data.setupTime
    ...
    created () {
      Util = this.$util;
      console.log(this.$store.state.examineInterval.station.info, 88)
