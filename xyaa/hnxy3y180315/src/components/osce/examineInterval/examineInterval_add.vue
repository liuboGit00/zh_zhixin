<template>
  ...
  <div style="height:100%;" id="stationMain" ref="stationMain">
    ...
    <div class="editForm">
      <el-form :model="info" :rules="rules" ref="info" label-width="84px" class="resetFormItemHeight">
        <el-row class="stationInfo">
          ...
          <template v-if="addType === 'STANDARD'">
            ...
            <el-col :span="6">
              <el-form-item label-width="0" prop="hasRemind">
                <span style="float:left;">是否有换站时间：</span>
                <el-radio-group v-model="hasTime" @change="changeHasTime">
            ...
            <el-col :span="6">
              <el-form-item label-width="0" prop="moveTime">
                ...
                <el-col :span="5">
                  <el-input :disabled="!hasTime" v-model="info.moveTime" size="small"></el-input>
                </el-col>
                <span>分钟</span>
              </el-form-item>
            </el-col>
          ...
          <el-col :span="5">
            <el-form-item label-width="0"  prop="isTeacherContent">
              <span style="float:left;">是否含考核内容、考官：</span>
              <el-radio class="radio" v-model="info.isTeacherContent" label="YES">是</el-radio>
              <el-radio class="radio" v-model="info.isTeacherContent" label="NO">否</el-radio>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <div style="overflow:hidden;">
      ...
      <el-col :span="8" :offset="1" v-if="info.isTeacherContent=='YES'">
        <!-- 考场管理员 -->
        <div class="el-form selectUserBox">
    ...
    <Modal :mask-closable="false" width="1000" v-model="setInfoModal" title="设置考站信息" class-name="vertical-center-modal">
      ...
      <set-info v-if="setInfoModal" :sceneType='info.sceneType'  @cancel="cancel" @done="updateScripts"></set-info>
      <div slot="footer"></div>
    </Modal>
    <!--设置考站SP信息-->
  ...
  export default {
    ...
    methods: {
      ...
      listenSubEvent (isLoadingFun) {
        console.log(this.info.moveTime,this.hasTime)
        if(this.info.moveTime==0&&this.hasTime&&this.addType === 'STANDARD'){
          this.errorMess('换站时间不能为0')
          return
        }
        // 检测数据
        if (!this.checkData()) {
...
//        return
        ...
        data.stationList.map(item => {
          item.roomSpecialtyList.map(ite => {
            if(ite.specialty){
              ite.specialty = ite.specialty.toString()
            }
            console.log(ite, 22)
          })
      ...
      viewTable () {
        ...
        let data=Util._.defaultsDeep({},this.getAjaxParamsData());
        data.stationList.map(item => {
          item.roomSpecialtyList.map(ite => {
            if(ite.specialty){
              ite.specialty = ite.specialty.join(',')
            }
          })
