<template>
  ...
  <div>
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="维修人：" prop="repairer">
              <el-input v-model="searchObj.repairer"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="维修时间：" prop="startStartTime">
              <date-group
                :dateGroup="{text:'',startDate:searchObj.startStartTime,endDate:searchObj.startEndTime}">
                <el-date-picker name="start" v-model="searchObj.startStartTime"
                                :editable="false" type="date" placeholder="选择日期"
                                :picker-options="pickerOptions0"
                                @change="handleStartTime"></el-date-picker>
                <span>-</span>
                <el-date-picker name="end" v-model="searchObj.startEndTime"
                                :editable="false" type="date" placeholder="选择日期"
    ...
    <div id="nosocomial" ref="tableData">
      <el-table v-if="Refresh" align="center" :context="self" :data="tableData"
              ...
              <el-table   :context="self" :data="scope.row.childAssetsList"   id="nosocomialSmall" :class="{'nosocomialSmallNoSmall':(!scope.row.childAssetsList||!scope.row.childAssetsList.length)}"
                ...
                <el-table-column label="操作" width="170" align="left">
                  <template scope="scope">
                    ...
                    <el-button size="small" type="warning" v-if="scope.row.assetsRepairingId&&showSelect"
                               @click="borrowThis(scope.row)"
                               :disabled="scope.row.repairingStatus=='REPAIRINGEND'">
                      维修结束
                    </el-button>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="维修人" prop="repairer"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column label="维修说明" prop="remarks"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column label="维修日期" prop="startTime" show-overflow-tooltip>
                  <template scope="scope">
                    <!--<p class="tagTime">-->
                    <!--<el-tag close-transition type="primary">开始：-->
                    {{ scope.row.startTime
                ...
                <el-table-column label="状态" prop="repairingStatus"
                                 show-overflow-tooltip>
                  <template scope="scope">
        ...
        <el-table-column label="操作" width="170" align="left">
          <template scope="scope">
            ...
            <el-button size="small" type="warning"
                       @click="borrowThis(scope.row)" v-if="scope.row.assetsRepairingId&&showSelect"
                       :disabled="scope.row.repairingStatus=='REPAIRINGEND'">
              维修结束
            </el-button>
          </template>
        ...
        <el-table-column label="状态" prop="repairingStatus"
                         show-overflow-tooltip>
          <template scope="scope">
            <p>{{ scope.row.repairingStatus | typeText }}</p>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <Modal :mask-closable="false" close-on-click-modal="false" height="200"
           v-model="endModal" title="对话框标题"
           class-name="vertical-center-modal"
           :width="500">
      <modal-header slot="header" :content="button.endId"></modal-header>
      <div>
        <div class="remove">确认该资产已经维修结束了吗？</div>
        <el-row>
          <el-col :span="10" :offset="14">

              <el-button @click="send" type="primary">确定
              </el-button>
            <el-button class="but-col" @click="endModal=false">取消</el-button>
          </el-col>
        </el-row>
      </div>
      <div slot="footer"></div>
...
//      // 允许选择的行
//      canSelect(row) {
//        return row.repairingStatus !== 'REPAIRING'
//      },
      initSelect(){
        this.searchObj={
          assetType:'',
...
//        this.searchObj.startEndTime=this.conductDate(this.searchObj.startEndTime)
      ...
      show (row) {
        ...
        this.todoId = row.assetsRepairingId
        this.openModel('show')
      },
      // 取消
      cancel (targer) {
      ...
      remove () {
        if (this.isSelected()) {
          ...
          this.operailityDataDelete.map((item) => {
            ...
            if(item.repairingStatus=='REPAIRING'){
              temp=false;
            }

          })
          if(!temp){
            this.errorMess('维修中的资产不能删除');
            return
          }
      ...
      toChannel () {
        let url = '/api/assetsRepairing/exportExcel?'
        if(this.multipleSelection.length>0){ //勾选导出
          let arr=[]
          this.multipleSelection.map(item=>{
            arr.push(item.assetsRepairingId)
          })
          if(arr.length>0){
            url=url+'assetsRepairingIds='+arr.toString()
          }
