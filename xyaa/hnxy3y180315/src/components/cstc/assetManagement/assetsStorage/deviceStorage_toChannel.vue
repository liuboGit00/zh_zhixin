  ...
  <div>
    <el-row class="but-space">
      ...
      <el-col :span="16">
        ...
        <a :href="http">
          <el-button size="mini" type="primary">下载模板<i
            class="ivu-icon ivu-icon-arrow-down-a"></i></el-button>
        </a>
      </el-col>
    </el-row>
    <el-row class="but-space">
      ...
      <el-col :span="16">
        <p>上传填好的资产信息表 ( 仅支持.xls/.xlsx格式)</p>
        </br>
        <xlsx @on-select-file="handleSelectedFile">上传</xlsx>
        <!--:row="1"-->
    ...
    <!--导入-->
    <Modal close-on-click-modal="false" width="1100" v-model="toChannelModal"
           title="对话框标题" class-name="vertical-center-modal">
      <modal-header slot="header" :content="toChannelId"></modal-header>
      <to-channel v-if="toChannelModal" :date="date" @cancel="closeChannel" :commitBefFun="commitBefFun"
                  :url="url.excelImportSave.path" @success="subHandelEvent"
                  :data="convertedData" :format="format" :addData="addData"
                  :ch2en="ch2en"></to-channel>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
...
//          "guaranteeDate":"2016-12-31",
        ...
        date: ['purchaseTime', 'manufactureDate', 'guaranteeDate', 'purchaseDate' ,'warehousingTime'],
        addData: {
    ...
    methods: {
      ...
      init () {
        ...
        this.http = http + 'static/template/资产导入导出模版.xlsx'
      ...
      commitBefFun(data) {
        data.map(item => {
          item.status = this.statusLists[item.status]
            ? this.statusLists[item.status]
            : item.status
        })
          return data;
      },
