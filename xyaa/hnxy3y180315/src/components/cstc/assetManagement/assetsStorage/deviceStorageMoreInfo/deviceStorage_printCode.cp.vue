        ...
        <div>
          <template v-for="(listItem,index) in codeListPage">
            ...
            <div style='page-break-after: always;'
                 v-if="index < codeListPage.length && index > 0"></div>
            <div class="deviceStoragePrintCodePageItem" :key="index">
              <div v-for="(item,cIndex) in listItem"
                ...
                <div class="showUi">
                  ...
                  <p :class="[!show.name? 'setShowClass':'']">设备名称：{{show.name ?
                    item.deviceTypeName : ''}}</p>
                  <p :class="{'setShowClass':!show.num}">设备编号：{{show.num ?
                    item.deviceIdentifier : ''}}</p>
                  <p :class="[!show.location ? 'setShowClass':'']">
                    存放地点：{{show.location ? item.storageLocation :
                    ''}}</p>
