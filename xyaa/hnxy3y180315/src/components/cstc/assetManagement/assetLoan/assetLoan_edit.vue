<template>
  <!-- 修改借用记录 -->
  <div class="editForm">
    <el-form :model="formValidate" ref="formValidate" label-width="100px"
             :rules="rules">
      <el-row>
        ...
        <date-group
          :dateGroup="{text:'',startDate:formValidate.borrowTime,endDate:formValidate.returnTime}">
          <el-col :span="8" :offset="2" name="start">
            <el-form-item label="借出日期：" prop="borrowTime" required>
              <el-date-picker v-model="formValidate.borrowTime"
          ...
          <el-col :span="8" :offset="2" name="end">
            <el-form-item label="归还日期：" prop="returnTime">
              <el-date-picker v-model="formValidate.returnTime"
                              :editable="false" type="datetime"
                              placeholder="选择日期"
    ...
    methods: {
      ...
      listenSubEvent (isLoadingFun) {
        ...
        if (isSubmit) {
          ...
          data.borrowTime = this.conductDate(data.borrowTime,
            'yyyy-MM-dd HH:mm:ss')
          data.returnTime = this.conductDate(data.returnTime,
            'yyyy-MM-dd HH:mm:ss')
      ...
      selectDeviceTypeId (val) {
        this.formValidate.deviceTypeId = this.selectObj[val] &&
          this.selectObj[val].id
      },
      /*
       * 组件初始化入口
       * */
