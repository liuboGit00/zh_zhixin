<template>
  ...
  <div>
    ...
    <el-row v-show="searchMore">
      <el-form :inline="true" :model="searchObj" ref="searchObj"
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="转出经手人：" prop="outOperator">
              <el-input v-model="searchObj.outOperator"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <!--<el-form-item label="转出时间：" prop="startStartTime">-->
              <!--<el-date-picker name="start" v-model="searchObj.scrapTime"-->
                              ...
                              <!--@change="handleStartTime"></el-date-picker>-->
            <!--</el-form-item>-->
            <el-form-item label="转出时间：" prop="outTimeBegin">
            <date-group :dateGroup="{text:'',startDate:searchObj.outTimeBegin,endDate:searchObj.outTimeEnd}"
                        style="display: inline;">

                <el-date-picker v-model="searchObj.outTimeBegin" :editable="false" type="date" placeholder="选择日期"
                                :picker-options="pickerOptions0"
                                @change="handleStartTime"></el-date-picker>

                <span>-</span>
                <el-date-picker v-model="searchObj.outTimeEnd" :editable="false" type="date" placeholder="选择日期"
        ...
        <el-row>
          <el-col :span="6">
            <el-form-item label="转出部门：" prop="outDepartment">
              <el-input v-model="searchObj.outDepartment"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            ...
            <span v-if="scope.row.warehousingTime">
              ...
              <!--{{Number(new Date().getFullYear())-Number(scope.row.warehousingTime.split('-')[0])}}-->
                ...
                <el-table-column label="存放地点" prop="storageLocation"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column label="转出部门" prop="outDepartment"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column label="经手人" prop="outOperator"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column label="转出时间" prop="outTime"
                                 show-overflow-tooltip></el-table-column>
    ...
    <!-- 分页按钮 -->
    <div>
      <span style="font-size: 14px;font-weight: bold;line-height: 35px">{{ makeSmall? small.num:big.num }}台/件，{{ makeSmall? small.money:big.money }}元</span>
    ...
    <Modal close-on-click-modal="false" height="200" v-model="removeModal"
      ...
      <remove v-if="removeModal" :deleteUrl="api.delete" @remove="subCallback" idKey="assetsOutId"
...
<script>
  ...
  // 模态框
  import add from './assetsOut_add.vue' // 新增
  import edit from './assetsOut_edit.vue' // 编辑
  import show from './assetsScrap_view.vue' // 查看
  ...
  export default {
    ...
    methods: {
      ...
      setTableData (isLoading) {
        ...
        params.outTimeBegin = this.conductDate(params. outTimeBegin,
          'yyyy-MM-dd')
        params.outTimeEnd = this.conductDate(params.outTimeEnd,
          'yyyy-MM-dd')
        ...
        getNum.url='assetsOut/statistics'
...
//        })
      ...
      toChannel () {
        let url = '/api/assetsOut/exportExcel?'
        if(this.multipleSelection.length>0){ //勾选导出
          let arr=[]
          this.multipleSelection.map(item=>{
            arr.push(item.assetsOutId)
          })
          if(arr.length>0){
            url=url+'assetsOutIds='+arr.toString()
          }
          ...
          this.searchObj.warehousingStartTime=this.conductDate(this.searchObj.warehousingStartTime)
          this.searchObj.warehousingEndTime=this.conductDate(this.searchObj.warehousingEndTime)
          this.searchObj.outTimeBegin = this.conductDate(this.searchObj.outTimeBegin,
            'yyyy-MM-dd')
          this.searchObj.outTimeEnd = this.conductDate(this.searchObj.outTimeEnd,
            'yyyy-MM-dd')
    ...
    watch: {
      ...
      tableData(){
        this.$nextTick(function () {
          ...
          console.log($('.nosocomialSmallNoSmall'),9999)
