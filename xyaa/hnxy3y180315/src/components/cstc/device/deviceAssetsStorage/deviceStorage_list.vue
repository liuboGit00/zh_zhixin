<template>
  ...
  <div id="nosocomial" ref="nosocomial">
    <!--<el-row>-->
    <ab-table-search :contenHeight="contenHeight-40" :loadTableData="loadTableData" :tableURL="tableURL" :selectSon=true :makeSmall='searchObj.isSubdivided'
      ...
      <template slot="search">
        ...
        <el-form-item label="单价>：">
          <el-input v-model="searchObj.price"></el-input>万元
        </el-form-item>
        ...
        <el-form-item label="使用年限：">
          <el-input type="number" v-model="searchObj.directionUse"></el-input>年
        </el-form-item>
      ...
      <template slot="tableItem">
        ...
        <el-table-column label="资产分类" prop="treeName" show-overflow-tooltip width="100"></el-table-column>
        ...
        <el-table-column label="购置日期" prop="purchaseDate" show-overflow-tooltip width="100"></el-table-column>
        ...
        <el-table-column label="经费来源" prop="fundsSource" show-overflow-tooltip width="100"></el-table-column>
      ...
      <template slot="selectSon">
        <span style="font-size: 14px;font-weight: bold;line-height: 35px;position: absolute;z-index:100;">总计：{{ makeSmall? small.num:big.num }}台/件，{{ makeSmall? small.money/10000:big.num/10000 }}万元</span>
      </template>
    </ab-table-search>
    <!--<el-col :xs="24" :md="14">-->
    <!--&lt;!&ndash; 操作按钮 &ndash;&gt;-->
    ...
    <Modal :mask-closable="false" v-model="storageModal" height="200" title="对话框标题"
      ...
      <storage v-if="storageModal" @cancel="cancel" @storage="subCallback" :operaility-data="operailityData" :depId="depId" :depMes="depMes"
    ...
    <Modal :mask-closable="false" v-model="editModal" height="200" title="对话框标题"
      ...
      <storage v-if="editModal" @cancel="cancel" @edit="subCallback" :operaility-data="operailityData" :depId="depId" :depMes="depMes"
...
//        this.setTableData();
       ...
       */
      handleSelectionChange (val,valList,type) {
//        console.log(type,this.multipleSelection,val)
        console.log(val,valList,420)
//        if(!type){
...
//        }else {
          ...
          valList.map((item)=>{
            item.childAssetsList&&item.childAssetsList.map((itemVal)=>{
              val.map((retuenList)=>{
                if(itemVal.id==retuenList.id){
                  console.log(itemVal.id)
                  temp=item;
                }
              })
            })
      ...
      //  }
        console.log(this.multipleSelection,999999)
        this.operailityDataDelete =this.$util._.defaultsDeep([],this.multipleSelection);
        this.operailityDataDelete.map((item)=>{
          if(item.childAssetsList&&item.childAssetsList.length){
            item.childAssetsList.map((item2)=>{
              item.id+=(','+item2.assetsId);
            })
          }
        })
      },
      ...
      excelExport () {
        if (this.isSelected()) {
          ...
          for (let i = 0; i < this.multipleSelection.length; i++) {
            assetsIds.push(this.multipleSelection[i].id)
          }
          assetsIds= assetsIds.join(',')
          let str=''
          for(var i in this.searchObj){
                str+=(i+'='+this.searchObj[i]+'&')
          }
          // let http = this.$store.getters.getEnvPath.http;
          this.derUrl = '/api/assets/exportExcel?'+str+'assetsIds='+assetsIds;
