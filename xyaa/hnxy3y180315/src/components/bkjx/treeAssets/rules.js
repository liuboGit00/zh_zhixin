...
let contract = {
  contractNumber:baseRules.requiredNoEvent,
  contractName:baseRules.requiredNoEvent,
  firstParty:baseRules.requiredNoEvent,
  partyB:baseRules.requiredNoEvent,
  price:[baseRules.requiredNoEvent,baseRules.floatTwo],
  signDate:baseRules.requiredNoEvent,
 ...
 */
let deviceStorage = {
  assetName: baseRules.required, // 借用人 - 必填
  assetNumber:baseRules.required,
};
 ...
 * 设备管理 - 资产入库 - 设备详情
 */
let AssetsStorageMoreInfo = {
  fundsSource:baseRules.required,
  purchaseDate:baseRules.isDate,
  purchaseTime: baseRules.isDate, // 购买时间 - 必须为日期时间
  price: [baseRules.float(2)], // 单价 - 必须为数字
//  model: baseRules.requiredNoEvent, // 设备编号
  parentAssetsNumber: baseRules.required, // 设备编号 - 必填
  assetName: baseRules.required,
  assetNumber: baseRules.required,
  treeId: baseRules.selectId,
  standard: baseRules.required,
  model:baseRules.required,
  discardingTimeLimit: [baseRules.float(2)],
  undsSource:baseRules.required,
  storageLocation:baseRules.required,
  // manufactureDate: baseRules.isDate, // 生产日期 - 必须为日期时间
  // guaranteeDate: baseRules.isDate, // 保修截止日期 - 必须为日期时间
  // status: baseRules.selectText // 使用类型 - 必选
};
 ...
 */
let deviceLoan = {
  restituer:baseRules.required,
  baseId:baseRules.selectId,
  assetNumber:baseRules.required,
  ...
  repairer:baseRules.required,
  returnTime: [baseRules.isDate],
};
 ...
 */
let deviceServicing = {
  assetName: baseRules.selectText, // 设备名称 - 必选
  deviceTypeId: baseRules.selectId, // 设备编号 - 必选
  repairer: baseRules.required, // 费用 - 必填
  startTime: baseRules.isDate, // 维修开始时间 - 必须为日期时间
  remarks: baseRules.required,
  assetNumber: baseRules.required,
};
let deviceBreak = {
  assetName: baseRules.selectText, // 设备名称 - 必选
  deviceTypeId: baseRules.selectId, // 设备编号 - 必选
  breakageUser: baseRules.required, // 费用 - 必填
  breakageTime: baseRules.isDate, // 维修开始时间 - 必须为日期时间
  breakaegRemarks: baseRules.required,
  assetNumber: baseRules.required,
};
/**
 * 设备管理- 报废记录
...
let deviceScrap = {
  assetNumber:baseRules.required,
  baseId: baseRules.selectId,
  scrapRemarks: baseRules.required,
  discarder:baseRules.required,
  deviceId: baseRules.required, // 设备名称 - 必选
  deviceTypeId: baseRules.required, // 设备编号 - 必选
...
let deviceOut={
  assetNumber:baseRules.required,
  baseId: baseRules.selectId,
  outOperator:baseRules.required,
  outTime:baseRules.isDate,
  outDepartment:baseRules.required,
  outRemark:baseRules.required,

};
let deviceloss={
  baseId: baseRules.selectId,
  countTime:baseRules.isDate,
  countOperator:baseRules.required,
  countRemark:baseRules.required,
}
/**
 * 耗材管理 - 消耗品管理
 */
