...
<script>
  ...
  export default {
    //props接收父组件传递过来的数据
    props: ['operailityData', 'deptId','treeId'],
    data() {
      return {
        toChannelUrl: api.importFromExcelValidate +'?treeId='+ this.treeId,
        active: 0,
        //保存按钮基本信息
    ...
    mounted() {
      ...
      this.http = http + 'static/template/课程表导入模板.xlsx';
      this.init();
    },
    methods: {
      /*
