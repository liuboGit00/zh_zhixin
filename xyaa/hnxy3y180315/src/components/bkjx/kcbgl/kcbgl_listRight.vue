...
<template>
  <div>
    ...
    <el-row class="roomButtonList" ref="buttonList" style="margin-bottom: 10px">
      <el-col :span="8" >
        <el-button size="small" type="info" @click="add">新建</el-button>
        <el-button size="small" type="info" @click="edit">编辑</el-button>
        <el-button size="small" type="danger" @click="remove">删除</el-button>
        <el-button size="small" type="warning" @click="toChannel">导入</el-button>
        <el-button size="small" type="info" @click="derive">导出</el-button>
      </el-col>
    ...
    <!-- 模态框 导入 -->
    <Modal :mask-closable="false" v-model="toChannelModal" height="200" title="对话框标题" class-name="vertical-center-modal"
           :width="840">
      <modal-header slot="header" :content="button.toChannelId"></modal-header>
      <to-channel v-if="toChannelModal" :url="api" @cancel="cancel" @toChannel="subCallback"
                  :operaility-data="operailityData" :url-params="api.save" :treeId="treeId"></to-channel>
      <div slot="footer"></div>
    </Modal>
...
<script>
  ...
  import edit from './kcbgl_add'; // 编辑  增加
  import toChannel from './kcbgl_tochannel'
  ...
  export default {
    ...
    data () {
      return {
        ...
        operailityData: [],
        multipleSelection: [],
        excelExport:'/techCourse/exportExcel',
        excelExportUrl: '',
        // 操作id
        todoId: {},
        //* 按钮 *//
    ...
    methods: {
      ...
      derive () {
        ...
        deriveData = $.extend(this.searchObj,deriveData)
        console.log(deriveData)
        let params = Util.serializeParams(deriveData,"string");
